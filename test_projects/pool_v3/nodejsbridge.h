...
private:
    ...
    //т.к. по этим символам распознаються границы пар-значений и значения-массивы и значения-объекты.
    //кстати ответ может иметь значения массивы и объекты, но только на первом уровне, вложенными друг в друга не допускается.
    ...
    void parseNextJsonElement(QString&, QString&) const; // извлечь и трансвормировать очередной элемент json ответа
    void wrapValueQuotes(QString&) const; // обернуть элемент json подходящими кавычками
