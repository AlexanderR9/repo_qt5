...
//  3. (только на 1-м шаге) считываются настройки линии со страницы, на др. данные берутся из записи структуры.
//  4. (кроме 1-го шага) собирается инфа о текущих объемах обоих активов всей результирующей ликвидности позы (включая реварды) на данном шаге.
...
//  9. обновить текущие балансы кошелька, запомнить новые значения балансов пары после свопа и скорректировать ликвидность линии.
//  10. вычисляем p1 / p2 для открытия позы так что бы выполнились все условия настроек (диапазон и доли токенов)
//  11. открываем новую позицию, получаем TX_HASH, запоминаем тиковый диапазон.
//  12. проверяется статус выполненной операции.
//       если не ОК, то состояние линии не меняется, все отстается как было и сценарий прерывается.
//  13. запрашивается текущий список поз, и находится поза где адреса токенов/fee сопадают с пулом ,
//      а также ликвидность > 0 и совпадает тиковый диапазон. запоминаем PID (для возможности в будущем проверять состояние позы).
//  14. обновить текущие балансы кошелька, запомнить новые значения балансов пары после MINT.
//-----------------------------------------------------------------
...
protected slots:    
    ...
    void slotAddGasPriceField(QJsonObject&);
