...
/*
template <class T>
T calcCheckSum(char* data, uint size)
{
     T* idata=reinterpret_cast<T*>(data);
     qint8 one=0x55;
     T cs =0;
     for(T i=0;i<static_cast<T>(sizeof(T));i++)
     {
         cs |= one << 8*i;
     }
...
/*
  ...
  * @arg puchMsg - message to calculate CRC upon
  * @arg usDataLen - quantity of bytes in message
  * @return the CRC as a unsigned short type
quint16 MB_CRC16_cs(const unsigned char * puchMsg, quint16 usDataLen)
{
